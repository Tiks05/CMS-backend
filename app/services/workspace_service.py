...
from app.schemas.workspace_schema import AuthorApplyResult, AuthorStatsSchema, NoticeItemSchema, NewsListItemSchema, \
    SortItem, BookDetailSchema, ChapterInfoSchema, ChapterCreateSchema, VolumeItem, ChapterListResponse, ChapterItem, \
    ChapterUpdateSchema, ChapterDetailSchema
...
def get_notice_list(limit: int = 3) -> list[NoticeItemSchema]:
    records = (
        News.query
        .filter_by(type='active', is_notice=True)
    ...
    return [
        NoticeItemSchema(
            notice_url=f"{request.host_url.rstrip('/')}{item.notice_url}" if item.notice_url else "",
            title=item.title,
...
def save_book(data, cover_file: FileStorage = None):
    # 1. 封面上传处理
    if cover_file:
        relative_path, absolute_url = save_uploaded_image(
            file=cover_file,
            sub_folder='/covers'
        )
    ...
    book = Book(
        ...
        tags=data.tag.strip(),
        intro=data.introduction.strip() or "新作品出炉，欢迎大家前往番茄小说阅读我的作品，希望大家能够喜欢，你们的关注是我写作的动力，我会努力讲好每个故事！",
        hero=hero,
        cover_url=cover_url,
        status='连载中',           # 默认状态
...
def get_book_list_by_user(user_id: int) -> list[dict]:
    ...
    for book in books:
        ...
        result.append({
            "id": book.id,
            "title": book.title,
            "pic": f"{request.host_url.rstrip('/')}{book.cover_url}" if book.cover_url else "/src/assets/images/workspace/writer/default_cover.png",
...
def update_book_info(data, cover_file: FileStorage = None):
    ...
    # 2. 封面上传（如有则替换）
    if cover_file:
        relative_path, _ = save_uploaded_image(
            file=cover_file,
            sub_folder='/covers'
        )
