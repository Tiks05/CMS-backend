...
def create_app():
    ...
    # 自动根据环境变量切换配置
    env = os.getenv("FLASK_ENV", "development") # FLASK_ENV 用于判断当前运行模式，第二个参数载入不同的配置类（例如：是否开启调试、数据库连接、JWT密钥等）
    if env == "production":
    ...
    cors.init_app(app, resources={r"/api/*": {
        ...
        "allow_headers": ["Content-Type", "Authorization"], # 允许发送 application/json 类型的数据（表单/JSON 请求），允许携带token
