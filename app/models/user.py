...
class User(db.Model):
    ...
    role: Mapped[str] = mapped_column(
        String(16), nullable=True, comment='用户角色（user/author/admin）'
    )
    ...
    level: Mapped[int] = mapped_column(
        Integer, default=0, nullable=False, comment='作家数值等级（Lv.0 起）'
    )
    become_author_at: Mapped[DateTime] = mapped_column(
        DateTime, nullable=True, comment='成为作家的时间'
    )  # 新增字段
