...
class Follow(db.Model):
    ...
    follower_id: Mapped[int] = mapped_column(
        Integer, ForeignKey('user.id'), nullable=False, comment='关注者ID（谁去关注别人）'
    )
    followed_id: Mapped[int] = mapped_column(
        Integer, ForeignKey('user.id'), nullable=False, comment='被关注者ID（被谁关注）'
    )
    created_at: Mapped[DateTime] = mapped_column(
        DateTime, server_default=func.now(), comment='关注时间'
    )
