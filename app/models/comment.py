...
class Comment(db.Model):
    ...
    user_id: Mapped[int] = mapped_column(Integer, ForeignKey('user.id'), nullable=False, comment='评论者ID')
    book_id: Mapped[int] = mapped_column(Integer, ForeignKey('book.id'), nullable=False, comment='小说ID')
    parent_id: Mapped[int] = mapped_column(Integer, ForeignKey('comment.id'), nullable=True, comment='父评论ID（用于楼中楼回复）')
    reply_to_user_id: Mapped[int] = mapped_column(Integer, ForeignKey('user.id'), nullable=True, comment='被@用户ID')
    ...
    reply_to_user: Mapped["User"] = relationship("User", foreign_keys=[reply_to_user_id], backref="replied_comments")
